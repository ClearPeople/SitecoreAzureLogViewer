...
namespace Monty.Linq
{
    ...
    /// Modified by Vicent Galiana, To adap to project's need
    /// </remarks>
    public static class QueryResultCache
    {
        ...
        /// </summary>
        public static PaginationCache<T> CachedResults<T>(this CloudTableQuery<T> query,CacheItemPriority priority,
            TimeSpan slidingExpiration)
        {

            string key = Utility.ToMd5Fingerprint(query.ToString());

            // try to get the query result from the cache
            var result = HttpRuntime.Cache.Get(key) as PaginationCache<T>;

            if (result == null)
            {                
                result = new PaginationCache<T>();
            ...
            return HttpRuntime.Cache.Get(key) as PaginationCache<T>; ;
    ...
    public static class Evaluator
    {
        ...
        class SubtreeEvaluator : ExpressionVisitor
        {
            ...
            public override Expression Visit(Expression exp)
            {
                ...
                return exp.CanReduce ? base.Visit(exp) : exp;
    ...
    public class LocalCollectionExpander : ExpressionVisitor
    {
        ...
        {
            //return expression.CanReduce ? new LocalCollectionExpander().Visit(expression) : expression;
